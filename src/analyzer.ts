...
export interface TestChange {
  ...
  impactedBy?: string; // Path to the non-test file that caused indirect impact
}
...
export class PlaywrightTestImpactAnalyzer {
  ...
  analyze(baseCommit: string, headCommit: string): ImpactAnalysisResult {
    ...
    const directlyImpactedTests: TestChange[] = [];
    const indirectlyImpactedTests: TestChange[] = [];

    // Separate test files and non-test files
    const changedTestFiles: Array<{ path: string; changeType: 'added' | 'deleted' | 'modified' | 'renamed' }> = [];
    const changedNonTestFiles: Array<{ path: string; changeType: 'added' | 'deleted' | 'modified' | 'renamed' }> = [];
    ...
    for (const file of changedFiles) {
      ...
      if (isTestFile) {
        ...
        const testChanges = this.analyzeTestFileChanges(
          baseCommit,
          headCommit,
          file.path
        );
    ...
    if (changedNonTestFiles.length > 0) {
      const indirectImpacts = this.analyzeIndirectImpact(
        baseCommit,
        headCommit,
        changedNonTestFiles,
        changedTestFiles
  ...
  private analyzeIndirectImpact(
    baseCommit: string,
    headCommit: string,
    changedNonTestFiles: Array<{ path: string; changeType: 'added' | 'deleted' | 'modified' | 'renamed' }>,
    changedTestFiles: Array<{ path: string; changeType: 'added' | 'deleted' | 'modified' | 'renamed' }>
  ): TestChange[] {
    const indirectImpacts: TestChange[] = [];
    ...
    const unchangedTestFiles = allTestFiles.filter(
      testFile => !changedTestFiles.some(cf => cf.path === testFile)
    );
    ...
    for (const testFile of unchangedTestFiles) {
      try {
        ...
        const testContent = execSync(
          `git show ${headCommit}:${testFile} 2>/dev/null`,
   ...
   */
  private getAllTestFiles(commit: string): string[] {
    try {
  ...
  private resolveImportPath(
    ...
    for (const testPath of possiblePaths) {
      try {
        execSync(
          `git cat-file -e ${commit}:${testPath} 2>/dev/null`,
          { cwd: this.repositoryPath, encoding: 'utf-8' }
        );
        return testPath;
      } catch {
  ...
  private getChangedFiles(
    baseCommit: string,
    headCommit: string
  ): Array<{ path: string; changeType: 'added' | 'deleted' | 'modified' | 'renamed' }> {
    try {
      const output = execSync(
   ...
   */
  private isPlaywrightTestFile(filePath: string): boolean {
    ...
    try {
      ...
      try {
        baseContent = execSync(
          `git show ${baseCommit}:${filePath} 2>/dev/null`,
          { cwd: this.repositoryPath, encoding: 'utf-8' }
        );
      } catch {
      ...
      try {
        headContent = execSync(
          `git show ${headCommit}:${filePath} 2>/dev/null`,
          { cwd: this.repositoryPath, encoding: 'utf-8' }
        );
      } catch {
      ...
      for (const headTest of headTests) {
        const matchingBaseTest = baseTests.find(
          (t) => t.testName === headTest.testName && t.kind === headTest.kind
        );
        if (!matchingBaseTest) {
          testChanges.push({
            type: 'added',
      ...
      for (const baseTest of baseTests) {
        const matchingHeadTest = headTests.find(
          (t) => t.testName === baseTest.testName && t.kind === baseTest.kind
        );
        if (!matchingHeadTest) {
          testChanges.push({
            type: 'removed',
  ...
  private extractTestsFromContent(
    content: string,
    filePath: string
  ): Array<{ testName: string; lineNumber: number; content: string; kind: TestChangeType }> {
    const tests: Array<{ testName: string; lineNumber: number; content: string; kind: TestChangeType }> = [];
    const lines = content.split('\n');
    ...
    const testPatterns = [
      ...
      // test("name", ...)
      // test(`name`, ...)
      { pattern: /test\s*\(\s*['"`](.+?)['"`]/g, kind: 'test' as TestChangeType },
      // test.only('name', ...)
      { pattern: /test\.only\s*\(\s*['"`](.+?)['"`]/g, kind: 'test' as TestChangeType },
      // test.skip('name', ...)
      { pattern: /test\.skip\s*\(\s*['"`](.+?)['"`]/g, kind: 'test' as TestChangeType },
    ];

    // Test suite patterns (describe blocks)
    const describePatterns = [
      // test.describe('name', ...)
      { pattern: /test\.describe\s*\(\s*['"`](.+?)['"`]/g, kind: 'describe' as TestChangeType },
      // test.describe.only('name', ...)
      { pattern: /test\.describe\.only\s*\(\s*['"`](.+?)['"`]/g, kind: 'describe' as TestChangeType },
      // test.describe.skip('name', ...)
      { pattern: /test\.describe\.skip\s*\(\s*['"`](.+?)['"`]/g, kind: 'describe' as TestChangeType },
    ];
   ...
   */
  private extractTestBody(lines: string[], startLine: number): string {
