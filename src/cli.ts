...
import type { ImpactAnalysisResult } from './analyzer.js';
...
program
  ...
  .description(
    'CLI tool to analyze which Playwright tests are impacted by changes between Git commits'
  )
  .version('1.0.0');
...
program
  ...
  .action(async (options) => {
    try {
      console.log(
        chalk.blue(
          `Analyzing impact between ${options.base} and ${options.head}...`
        )
      );
...
interface RunnableTest {
  filePath: string;
  testName: string;
  impactType: 'added' | 'removed' | 'modified';
  changeKind: 'test' | 'describe';
  lineNumber?: number;
  source: 'direct' | 'indirect';
  impactedBy?: string;
}

interface RunnableOutput {
  summary: {
  ...
  impactedTests: RunnableTest[];
  playwrightCommands: {
    runAll: string;
    runByFile: string;
    runByGrep: string;
    runAddedOnly: string;
    runModifiedOnly: string;
  };
  fileList: string[];
  grepPattern: string;
}

function outputJson(result: ImpactAnalysisResult, outputFile?: string) {
...
function generateRunnableOutput(result: ImpactAnalysisResult): RunnableOutput {
  const impactedTests: RunnableTest[] = [];
...
function outputRunnable(result: ImpactAnalysisResult, outputFile?: string) {
...
function outputTable(result: ImpactAnalysisResult) {
  console.log('\n' + chalk.bold('='.repeat(80)));
  ...
  console.log(
    `  Tests added: ${chalk.green(result.summary.testsAdded)}`
  );
  console.log(
    `  Tests removed: ${chalk.red(result.summary.testsRemoved)}`
  );
  console.log(
    `  Tests modified: ${chalk.yellow(result.summary.testsModified)}`
  );
  ...
  if (result.directlyImpactedTests.length > 0) {
    ...
    for (const [filePath, tests] of Object.entries(groupedByFile)) {
      ...
      for (const test of tests) {
        ...
        const lineInfo = test.lineNumber ? `:${test.lineNumber}` : '';
        console.log(
          `    ${icon} ${kindIcon} ${test.testName}${chalk.gray(lineInfo)}`
        );
      }
    }
  } else {
...
function groupTestsByFile(
  tests: Array<{ filePath: string; testName: string; type: string; lineNumber?: number; changeKind: string }>
): Record<string, Array<{ filePath: string; testName: string; type: string; lineNumber?: number; changeKind: string }>> {
  return tests.reduce<Record<string, Array<{ filePath: string; testName: string; type: string; lineNumber?: number; changeKind: string }>>>((acc, test) => {
